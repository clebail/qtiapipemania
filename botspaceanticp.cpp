...
void BotSpaceAnticp::ancrerDefausse(int col, int row, ESens entree) {
    ...
    if(caseAnticipee(col, row, entree, typePont, gc, gr, ge, &chaine) && (gc != fc || gr != fr)) {
        ancrageDefausse = gr * p->getLargeur() + gc;
        // L'entree compte autant que la case : c'est elle qui dit quelles
        // pieces y ont leur place. Poser la le bon type dans le mauvais sens,
        // c'est boucher le trajet qu'on essayait de preparer.
        ancrageEntree = ge;

        // Et le pari se juge avec tout ce sur quoi il repose. La projection
        // vient de supposer deux pieces que le plateau n'a pas : le pont sur la
        // tete -- pris dans la file, ou impose par pontForce quand la tete est
        // obligee -- et le premier type qui se raccorde sur le rang 1. Les
        // laisser hors du test, c'est demander a defausser() si le pari a de la
        // place en lui montrant un plateau ou la tete et le rang 1 sont encore
        // vides : de la place, il en voit forcement.
        //
        // Le reste du trajet, lui, est deja pose ; il part quand meme avec, car
        // le flux l'aura traverse et ces cases ne sont pas une reserve.
        ancrageAmont << qMakePair(row * p->getLargeur() + col, typePont)
                     << qMakePair(fr * p->getLargeur() + fc, possibles.first());
        ancrageChaine = chaine;
    }
